...
public class MainApp {
	...
	// @Autowiring("java.lang.String") /* <- throws 'Class specified in
	// annotation is not compatible' exception*/
	...
	private static class ObjectInfo {
		...
		public String toString() {
			...
			for (Method m : methods) {
				...
				// In order to get formal method parameters names via
				// reflection, they must be present in .class file.
				// By default Java compiler discards this information,
				// so you'll get arg0, arg1... instead of real informative
				// names.
				// In order to have the method parameters names included in the
				// .class files,
				// you must compile .java files with -parameters compile option.
				// To do this in Eclipse, go to Window > Preferences > Java >
				// Compiler and
				// check the box 'Store information about method parameters
				// (usable via reflection)'
	...
	public static void main(String[] args) {
		...
		Transport bus = (Bus) factory.getBean("bus", Transport.class);
		bus.getTransport();

		Transport bus2 = (Bus) factory.getBean("bus2", Transport.class);
		bus2.getTransport();
		...
		System.out.println("==========");
		System.out.println(new ObjectInfo(bus.toString(), "String representation of the 'bus' object").toString());

		// ================
		// ================ CUSTOM INTERCEPTOR AUTOWIRING THROUGH ANNOTATION
		// DEMO
		// ================
		// (to prevent recompiling by Eclipse, switch Project > Build
		// Automatically off)
		...
		// This block is needed for being able to inspect currently loaded
		// classes
