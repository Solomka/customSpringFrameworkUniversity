		...
		for (Field currentField : fields) {
			if (currentField.isAnnotationPresent(Autowiring.class)) {
				...
				if (!currentField.getAnnotation(Autowiring.class).value().isEmpty()) {
					...
					try {
						classInAnnotation = Class.forName(currentField.getAnnotation(Autowiring.class).value());
					...
					if (!classes.stream().anyMatch(canInstantiate(currentFieldClass))) {
						throw new ConfigurationException("No suitable implementation for " + currentFieldClass.getName()
					...
					if (classes.stream()
							.anyMatch(canInstantiate(currentFieldClass).and(isTheSameClassAs(match).negate()))) {
						throw new ConfigurationException("Ambiguous configuration for " + currentFieldClass.getName()
